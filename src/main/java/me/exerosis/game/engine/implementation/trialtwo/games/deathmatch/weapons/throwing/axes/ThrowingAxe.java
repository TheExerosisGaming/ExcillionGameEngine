...
public class ThrowingAxe extends Weapon implements Runnable {
    ...
    private HashMap<ArmorStand, Player> _axes = new HashMap<>();
    private int _slot;

    public ThrowingAxe(String name, Game game, SpectateComponent spectateComponent, ThrowingAxeCooldown axeCooldown, Axeman axeman) {
        super(name, game, spectateComponent);
        _axeCooldown = axeCooldown;
        _axeman = axeman;
    }
    ...
    public void onGameStateChange(GameStateChangeEvent event) {
        if (event.getNewGameState().equals(GameState.POST_GAME))
            _axes.keySet().forEach(ArmorStand::remove);
    }
    ...
    private void finishGroundPose(ArmorStand stand) {
        ...
        location = location.add(0.0, -height, 0.0);
